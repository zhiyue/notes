...
- Loading features from dicts # 我们通常用字典代表表示稀疏特征矩阵，sklearn提供的'DictVectorizer'可以将字典转换称为特征向量，并且这个特征向量是为稀疏矩阵优化存储的。DictVectorizer class uses a scipy.sparse matrix by default instead of a numpy.ndarray. # 使用scipy里面的CSR(Compressed Sparse Row format)来存储特征向量。
- Feature hashing
- Text feature extraction
- Image feature extraction
  - Patch extraction # 能够将图像分成patch取出
...
# 这里使用的是standardization方法
# 注意scale是作用在同一个column上

...
*** Layer
caffe提供了许多内置layer，比如convolution layer, pool layer, dropout layer, nonlinearity layer等。这些层说明以及具体参数都可以在 [[http://caffe.berkeleyvision.org/tutorial/layers.html][这里]] 查到（文档比代码有一些滞后，文档里面没有说支持了dropout但是实际已经提供）。每个layer有输入一些'bottom' blobs, 输出一些'top' blobs.
...
todo(dirlt): deploy和train.prototxt差别.
...
note(dirlt): solver可以指定net, train_net和test_nets. 如果不想训练时候

...
** Abstract
** Introduction
note(dirlt): 相对于 [[file:./dremel.org][Dremel]] 来说, PowerDrill针对更小的数据量但是需要达到更加及时的响应. [[file:./dremel.org][Dremel]] 是用来作为内部数据仓库使用的, 响应时间在1~2min, 而PowerDrill则是用来对外服务的(比如GA), 响应时间在~10s.

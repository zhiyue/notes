...
指令是不区分大小写的, 然而约定是大写的以便区分其他的参数。Docker会读测试Dockerfile中的指令，第一条命令必须是'FROM'来指定你正在构建的基本镜像。
   ...
   - CMD <command> # 容器启动时候默认的shell命令
   - EXPOSE <port> [<port>...] # 暴露外部端口
   - ENV <key> <value> # 设置环境变量
   - ADD <src> <dest> # 添加hostOS文件。权限是755,uid=0.
   - ENTRYPOINT <command> # 和CMD类似
   - VOLUME # 挂载到本地或者是其他container文件系统
   - USER # 设置镜像运行用户名和uid
   - WORKDIR # 初始工作目录

** troubleshooting
*** 如何为container分配固定IP
现在除了在启动container时候使用lxc-conf指定lxc配置之外没有特别好的办法。此外有个工具可以简化这个步骤 https://github.com/jpetazzo/pipework

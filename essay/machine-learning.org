...
- Algorithm Tuning # 算法调优。假设我们已经选择好了基本算法之后通过gridsearch配合CV来寻找最佳参数。这个过程最好做成自动化形式并且搜索参数不要太细粒度（I strongly advise that the process is automated and reasonably coarse grained as you can quickly reach points of diminishing returns (fractional percentage performance increases) that may not translate to the production system.）我理解是如果这个参数搜索空间过于细粒度的话，会耗费训练资源但是改进比例却非常小。
- Ensembles # 增强方法。Bagging是使用一个算法在不同数据集合上训练然后平均组合；Boosting是使用一个算法在一个训练数据集合上不断改进得到不同假设进行线性组合；Blending则是使用将不同假设得到的结果组合起来得到系数然后使用这个系数来组合假设。
- Extreme Feature Engineering # 特征工程。This strategy is about exposing more structure in the problem for the algorithms to learn. 主要目的就是为了将问题中更多的结构信息暴露给算法。
